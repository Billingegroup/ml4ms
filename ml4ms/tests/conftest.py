...
from ml4ms.schemas import load_exemplars
...
def make_db():
    """A test fixture that creates and destroys a temporary database
...
#         )
#     if os.name == 'nt':
#         # If on windows, the mongod command cannot be run with the fork or syslog options. Instead,
#         it is installed as
#         # a service and the exceptions that would typically be log
#         outputs are handled by the exception handlers below.
#         # In addition, the database must always be manually deleted
#         from the windows mongo instance before running a
#         # fresh test.
...
#             print(
#                 "Mongodb likely has not been installed as a service.
#                 In order to run mongodb tests, make sure\n"
#                 "to install the mongodb community edition with the
#                 following link: \n"
...
#         subprocess.check_call(cmd, cwd=repo)
#     except CalledProcessError:
#         print("If using linux or mac, Mongod command failed to execute. If using windows, the status of
#         mongo could \n"
#               "not be retrieved. In order to run mongodb tests, make sure to install the mongodb community
#               edition with"
#               "\nthe following link:\n"
#               "https://docs.mongodb.com/manual/installation/")
#         yield False
...
#         )
#     if os.name == 'nt':
#         # If on windows, the mongod command cannot be run with the fork or syslog options. Instead, it is
#         installed as
#         # a service and the exceptions that would typically be log outputs are handled by the exception handlers
#         below.
#         # In addition, the database must always be manually deleted from the windows mongo instance before
#         running a
#         # fresh test.
#         cmd = ["mongo", REGOLITH_MONGODB_NAME, "--eval", "db.dropDatabase()"]
#         try:
...
#         subprocess.check_call(cmd, cwd=repo)
#     except CalledProcessError:
#         print("If on linux/mac, Mongod command failed to execute. If on windows, the status of
#         mongo could not be\n"
#               "retrieved. In order to run mongodb tests, make sure to
#               install the mongodb community edition with\n"
#               "the following link:\n"
#               "https://docs.mongodb.com/manual/installation/")
#         yield False
#         return
...
#         except CalledProcessError:
#             print(
#                 "Mongod likely has not been installed as a service. In order to run mongodb tests,
#                 make sure\n"
#                 "to install the mongodb community edition with the following link: \n"
#                 "https://docs.mongodb.com/manual/installation/")
#             yield False
...
#         print("If on linux/mac, Mongod command failed to execute. If on windows, the status of
#         mongo could not be\n"
#               "retrieved. In order to run mongodb tests, make sure to install the mongodb community
#               edition with\n"
#               "the following link:\n"
#               "https://docs.mongodb.com/manual/installation/")
#         yield False
...
def exemplars_to_fs_json(fspath, collection_list=None):
    ...
    for coll, example in exemplars.items():
        if isinstance(example, list):
            ...
            d = {example["_id"]: example}
        dump_json_collection(f"{coll}.json", d)
    os.chdir(cwd)
